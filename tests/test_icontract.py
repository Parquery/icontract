...
import icontract2
from icontract2 import requires, ensures, invariant
...
class TestUnwindDecoratorStack(unittest.TestCase):
    def test_wo_decorators(self):
        ...
        self.assertListEqual([0], [a_func() for a_func in icontract2._unwind_decorator_stack(func)])
    ...
    def test_with_single_decorator(self):
        ...
        self.assertListEqual([1, 0], [a_func() for a_func in icontract2._unwind_decorator_stack(func)])
    ...
    def test_with_double_decorator(self):
        ...
        self.assertListEqual([3, 1, 0], [a_func() for a_func in icontract2._unwind_decorator_stack(func)])


class TestPrecondition(unittest.TestCase):
    def test_ok(self):
        @requires(lambda x: x > 3)
    ...
    def test_fail(self):
        ...
        pre_err = None  # type: Optional[icontract2.ViolationError]
        try:
            some_func(x=1)
    ...
    def test_fail_with_description(self):
        @requires(lambda x: x > 3, "x must not be small")
        def some_func(x: int, y: int = 5) -> None:
            pass
    ...
    def test_with_pathlib(self):
        @requires(lambda path: path.exists())
    ...
    def test_with_stacked_decorators(self):
        ...
        @mydecorator
        @requires(lambda x: x < some_var)
        @requires(lambda x: x > another_var)
        @mydecorator
        def some_func(x: int) -> str:
            return str(x)
    ...
    def test_with_default_values(self):
        @requires(lambda a: a < 10)
        @requires(lambda b: b < 10)
        @requires(lambda c: c < 10)
    ...
    def test_benchmark_when_disabled(self):
        @requires(lambda x: x > 3, enabled=False)
        def pow_with_pre(x: int, y: int) -> int:
            return x**y
    ...
    def test_repr_args(self):
        @requires(lambda x: x > 3, repr_args=lambda x: "x was {:03}".format(x))
        def some_func(x: int, y: int = 5) -> None:
            pass
    ...
    def test_repr_args_unexpected_arguments(self):
        ...
        try:
            ...
            @requires(lambda x: x > 3, repr_args=lambda z: "z was {:X}".format(z))
            def some_func(x: int, y: int = 5) -> None:
                pass
    ...
    def test_repr(self):
        ...
        @requires(lambda x: len(x) < 10, a_repr=a_repr)
    ...
    def test_class(self):
        class A:
            ...
            @requires(lambda self: self.y > 10, repr_args=lambda self: "self.y was {}".format(self.y))
    ...
    def test_repr_nested_property(self):
        ...
        class A:
            ...
            @requires(lambda self: self.b.x > 0)
    ...
    def test_repr_nested_method(self):
        ...
        class A:
            ...
            @requires(lambda self: pathlib.Path(str(gt_zero(self.b.c(x=0).x() + 12.2 * z))) is None)
            def some_func(self) -> None:
                pass
    ...
    def test_repr_value_global(self):
        @requires(lambda x: x < SOME_GLOBAL_CONSTANT)
        def some_func(x: int) -> None:
            pass
    ...
    def test_repr_value_closure_and_global(self):
        ...
        @requires(lambda x: x < y + SOME_GLOBAL_CONSTANT)
        def some_func(x: int) -> None:
            pass
    ...
    def test_enabled(self):
        @requires(lambda x: x > 10, enabled=False)
    ...
    def test_property_getter(self):
        class SomeClass:
            ...
            @property
            @requires(lambda self: self._some_prop > 0)
    ...
    def test_property_deleter(self):
        class SomeClass:
            ...
            @some_prop.deleter
            @requires(lambda self: self.some_prop > 0)
...
class TestPostcondition(unittest.TestCase):
    ...
    def test_fail(self):
        ...
        post_err = None  # type: Optional[icontract2.ViolationError]
        try:
            some_func(x=1)
        except icontract2.ViolationError as err:
            post_err = err
    ...
    def test_fail_with_description(self):
        @ensures(lambda result, x: result > x, "expected summation")
        def some_func(x: int, y: int = 5) -> int:
            return x - y
    ...
    def test_with_stacked_decorators(self):
        ...
        @mydecorator
        @ensures(lambda result, x: x < result + some_var)
        @ensures(lambda result, y: y > result + another_var)
        @mydecorator
    ...
    def test_with_default_values_outer(self):
        @ensures(lambda result, c: result % c == 0)
        @ensures(lambda result, b: result < b)
    ...
    def test_repr(self):
        ...
        @ensures(lambda result, x: len(result) > x, a_repr=a_repr)
    ...
    def test_enabled(self):
        @ensures(lambda x, result: x > result, enabled=False)
        def some_func(x: int) -> int:
            return 123
    ...
    def test_invalid_postcondition_arguments(self):
        @ensures(lambda b, result: b > result)
        def some_function(a: int) -> None:  # pylint: disable=unused-variable
            pass
    ...
    def test_postcondition_in_abstract_static_method(self):
        class SomeAbstract(icontract2.DBC):
            @staticmethod
            @ensures(lambda result: result != 0)
            def some_func(x: int) -> int:
                pass
    ...
    def test_property_setter(self):
        class SomeClass:
            ...
            @some_prop.setter
            @ensures(lambda self: self.some_prop > 0)
            def some_prop(self, value: int) -> None:
                pass
...
class TestSlow(unittest.TestCase):
    def test_slow_set(self):
        self.assertTrue(icontract2.SLOW,
                        "icontract2.SLOW was not set. Please check if you set the environment variable ICONTRACT_SLOW "
...
class TestInvariant(unittest.TestCase):
    ...
    def test_multiple_invs_first_checked(self):
        @invariant(lambda self: self.x > 0)
        @invariant(lambda self: self.x < 10)
        class SomeClass:
            def __init__(self) -> None:
                self.x = -1
    ...
    def test_inv_with_invalid_arguments(self):
        ...
        try:
            ...
            @invariant(lambda self, z: self.x > z)
            class _:
                def __init__(self) -> None:
                    self.x = 100
    ...
    def test_inv_disabled(self):
        @invariant(lambda self: self.x > 0, enabled=False)
        class SomeClass:
            def __init__(self) -> None:
                self.x = -1
    ...
    @unittest.skip("Skipped the benchmark, execute manually on a prepared benchmark machine.")
    def test_benchmark_when_disabled(self):
        @invariant(lambda self: bool(time.sleep(5)), enabled=False)
        class SomeClass:
            def __init__(self) -> None:
                self.x = 100
    ...
    def test_property_getter(self):
        @invariant(lambda self: not self.toggled)
        class SomeClass:
            def __init__(self) -> None:
                self.toggled = False
...
class TestPreconditionInheritance(unittest.TestCase):
    ...
    def test_require_else(self):
        class A(icontract2.DBC):
            @requires(lambda x: x % 2 == 0)
            def func(self, x: int) -> None:
                pass

        class B(A):
            @requires(lambda x: x % 3 == 0)
            def func(self, x: int) -> None:
                pass
    ...
    def test_that_base_preconditions_apply_to_init_if_not_defined(self):
        ...
        icontract_violation_error = None  # Optional[icontract2.ViolationError]
        try:
            _ = B(x=-1)
        except icontract2.ViolationError as err:
            icontract_violation_error = err
...
class TestPreconditionInheritanceProperty(unittest.TestCase):
    def test_getter_setter_deleter_valid(self):
        class SomeBase(icontract2.DBC):
            ...
            @some_prop.deleter
            @requires(lambda self: not self.deleted)
            def some_prop(self) -> None:
                self.deleted = True
    ...
    def test_getter(self):
        class SomeBase(icontract2.DBC):
            ...
            @property
            @requires(lambda self: not self.toggled)
            def some_prop(self) -> int:
                return 0
    ...
    def test_setter(self):
        ...
        self.assertIsNotNone(icontract_violation_error)
        self.assertEqual('value > 0: value was 0', str(icontract_violation_error))
...
class TestPostconditionInheritance(unittest.TestCase):
    def test_inherited_without_implementation(self):
        class A(icontract2.DBC):
            @ensures(lambda result: result < 100)
            def func(self) -> int:
                return 1000
    ...
    def test_ensure_then(self):
        class A(icontract2.DBC):
            @ensures(lambda result: result % 2 == 0)
            def func(self) -> int:
                return 10

        class B(A):
            @ensures(lambda result: result % 3 == 0)
            def func(self) -> int:
                return 6
...
class TestPostconditionInheritanceProperty(unittest.TestCase):
    def test_getter_setter_deleter_valid(self):
        class SomeBase(icontract2.DBC):
            ...
            @property
            @ensures(lambda self, result: self._some_prop == result)
            def some_prop(self) -> int:
                return self._some_prop

            @some_prop.setter
            @ensures(lambda self, value: self.some_prop == value)
            def some_prop(self, value: int) -> None:
                self._some_prop = value

            @some_prop.deleter
            @ensures(lambda self: self.deleted)
            def some_prop(self) -> None:
                self.deleted = True
    ...
    def test_getter(self):
        class SomeBase(icontract2.DBC):
            ...
            @property
            @ensures(lambda self: not self.toggled)
            def some_prop(self) -> int:
                return 0
...
class TestInvariantInheritance(unittest.TestCase):
    ...
    def test_additional_invariant_that_fails_in_childs_init(self):
        ...
        @invariant(lambda self: self.x > 100)
        class B(A):
            def __repr__(self) -> str:
                return "instance of B"
